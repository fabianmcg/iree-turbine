...
# Symbols
M, BLOCK_M, ADDRESS_SPACE, ELEMENTS_PER_LOAD, ELEMENTS_PER_STORE, ITERATIONS_PER_THREAD_M = (
    tkl.sym.M,
    tkl.sym.BLOCK_M,
...
def build_block_constraints(*args, **kwargs) -> Sequence[tkw.Constraint]:
    ...
    constraints += [tkw.ThreadConstraint(M, ITERATIONS_PER_THREAD_M)]
    threads_per_block=kwargs["threads_per_block"] if "threads_per_block" in kwargs  else (1, 1, 1)
    constraints += [
...
def single_read(
    ...
    a_reg = tkw.read(a, elements_per_thread=ELEMENTS_PER_LOAD, manually_specified_indexing=OrderedDict([(M, 1)]),)
    tkw.write(a_reg, a, elements_per_thread=ELEMENTS_PER_STORE, manually_specified_indexing=OrderedDict([(M, 1)]),)
...
#     # Compiler crashes when threads_per_block does not divide WorkgroupConstraint.
#     try:
#         cfg = copy.deepcopy(static_config_1xwave_block_1xvector_1xload_1xstore())
#         cfg["threads_per_block"] = (2, 1, 1)
