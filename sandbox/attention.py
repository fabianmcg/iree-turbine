...
def attention(
    ...
    class Attention(nn.Module):
        def forward(self, q, k, v):
            return torch.nn.functional.scaled_dot_product_attention(
                q, k, v, is_causal=is_casual, attn_mask=None, enable_gqa=enable_gqa
            )
    ...
    iree_run = IREEModule.from_torch(model, (q, k, v), dynamic_shapes=dynamic_shapes)
...
def main():
    ...
    parser.add_argument(
        "-k1", type=int, required=True, help="embedding dimension of query and key"
    )
