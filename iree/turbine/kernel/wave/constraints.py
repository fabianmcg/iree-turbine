...
from .utils.symbol_utils import infer_static_shape, delinearize_index
...
def _check_wave_constraints_and_get_num_waves(
    ...
    for i, workgroup in enumerate(workgroup_constraints):
        ...
        vector_size = 1
        if (
            hardware_constraint.vector_shapes is not None
            and workgroup.dim in hardware_constraint.vector_shapes
        ):
            vector_size = hardware_constraint.vector_shapes[workgroup.dim]
        # Get the number of waves in the `workgroup.dim` dimension.
        num_waves[i] = ceiling(
            (workgroup.tile_size / waves_by_dim[workgroup.dim].tile_size) / vector_size
        )
...
def _simplify_wave_ids(
    wave_ids: list[IndexExpr], b_dim: tuple[int, int, int]
) -> list[IndexExpr]:
...
def initialize_and_check_constraints(
    ...
    wave_ids = _simplify_wave_ids(list(reversed(wave_ids)), hw_c.threads_per_block)
